    ...
    try:
        ...
        if (position['type'] is None) or (position['type']=='long' and buy_phase <= 1 and roe < -5) :
            ...
            if slow_k_30m[-2] <= 20 or (macd_30m[-1] > macd_signal_30m[-1] and macd_30m[-2] < macd_30m[-1]) :
                ...
                if slow_k_5m[-1] <= 20 :
                    # 
                    if slow_k_1m[-2] <= 25 and slow_k_1m[-2] <= slow_d_1m[-2] and slow_k_1m[-1] > slow_d_1m[-1] and  macd_1m[-2] < macd_1m[-1] :
                        position['type'] = 'long'
                        if buy_phase == 0 :
                            buy_phase = 1
                            buy_msg = "long 진입(1차)"
        ...
        if (position['type'] is None) or (position['type']=='short' and buy_phase <= 1 and roe < -5) :
            ...
            if slow_k_30m[-2] >= 80 or (macd_30m[-1] < macd_signal_30m[-1] and macd_30m[-2] > macd_30m[-1]) :
                ...
                if slow_k_5m[-1] >= 80   :
                    # 
                    if slow_k_1m[-2] >= 75 and slow_k_1m[-2] >= slow_d_1m[-2] and slow_k_1m[-1] < slow_d_1m[-1] and macd_1m[-2] > macd_1m[-1]  :
                        position['type'] = 'short'
                        if buy_phase == 0 :
                            buy_phase = 1
                            buy_msg = "short 진입(1차)"
