...
while True: 
    try:
        ...
        buy_status = 0
        # Enter  
        if ((position['type'] is None) or (position['type']=='long' and buy_phase <= 2 and roe < -2))  :
        ...
        if ((position['type'] is None) or (position['type']=='short' and buy_phase <= 2 and roe < -2))   :
        #if (position['type'] is None) :    and slow_k_30m[-2] > slow_k_30m[-1]
            #short조건1
        ...
        if buy_phase >= 1 :
            ...
            if position['type'] == 'long':  # or slow_k_30m[-1] <= slow_d_30m[-1] and slow_k_5m[-1] >= 50 and slow_k_1m[-2] >= 50
                if slow_k_30m[-1] >= 80 and slow_k_5m[-1] >= 80 and slow_k_1m[-2] >= 50 and slow_k_1m[-2] >= slow_d_1m[-2] and slow_k_1m[-1] < slow_d_1m[-1] and macd_osc_1m[-2] > macd_osc_1m[-1]  :
                    ...
                    buy_phase=0
                elif 8 <= buy_condition <= 9 and slow_k_5m[-2] >= slow_d_5m[-2] and slow_k_5m[-1] < slow_d_5m[-1] :
                    binance.create_market_sell_order(symbol=symbol, amount=amount)
                    time.sleep(1)
                    position['type'] = None 
