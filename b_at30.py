...
while True: 
    try:
        ...
        # 과거 조회
        btc_ohlcv_5m = binance.fetch_ohlcv(symbol="BTC/USDT", timeframe='5m', since=None, limit=200)
        ...
        btc_ohlcv_1m = binance.fetch_ohlcv(symbol="BTC/USDT", timeframe='1m', since=None, limit=200)
        ...
        if buy_phase >= 1 :
            ...
            if position['type'] == 'long':  # or slow_k_30m[-1] <= slow_d_30m[-1] and slow_k_5m[-1] >= 50 and slow_k_1m[-2] >= 50
                if buy_condition == 9 and slow_k_30m[-2] >= 80 and slow_k_5m[-2] > slow_k_5m[-1] and slow_k_1m[-2] >= slow_d_1m[-2] and slow_k_1m[-1] < slow_d_1m[-1] and macd_osc_1m[-2] > macd_osc_1m[-1]  :
                    ...
                    buy_phase=0
                elif buy_condition == 1 and slow_k_5m[-2] >= 30 and slow_k_5m[-2] >= slow_d_5m[-2] and slow_k_5m[-1] < slow_d_5m[-1] :
                    binance.create_market_sell_order(symbol=symbol, amount=amount)
                    time.sleep(1)
                    position['type'] = None 
                    bot.sendMessage(mc, "long 청산(4) : "+str(roe)+"%")
                    buy_phase=0
                elif buy_condition == 9 and (slow_k_5m[-2] >= 80 or slow_k_5m[-1] >= 80) and (slow_k_1m[-2] >= 80 or slow_k_1m[-1] >= 80) and slow_k_1m[-2] > slow_d_1m[-2] and slow_k_1m[-1] <= slow_d_1m[-1]  :
                    binance.create_market_sell_order(symbol=symbol, amount=amount)
                    time.sleep(1)
                    position['type'] = None 
                    bot.sendMessage(mc, "long 청산(5) : "+str(roe)+"%")
                    buy_phase=0
                elif buy_condition == 9 and (slow_k_1m[-2] >= 80 or slow_k_1m[-1] >= 80) and slow_k_1m[-2] >= slow_d_1m[-2] and slow_k_1m[-1] < slow_d_1m[-1] :
                    binance.create_market_sell_order(symbol=symbol, amount=amount)
                    time.sleep(1)
                    position['type'] = None 
            ...
            if position['type'] == 'short':  #   or slow_k_30m[-1] >= slow_d_30m[-1])
                if buy_condition == 9 and slow_k_30m[-2] <= 20 and slow_k_5m[-2] < slow_k_5m[-1] and slow_k_1m[-2] <= slow_d_1m[-2] and slow_k_1m[-1] > slow_d_1m[-1] and macd_osc_1m[-2] < macd_osc_1m[-1]  :
                    ...
                    buy_phase=0   
                elif buy_condition == 1 and slow_k_5m[-2] <= 70 and slow_k_5m[-2] <= slow_d_5m[-2] and slow_k_5m[-1] > slow_d_5m[-1] :
                    binance.create_market_buy_order(symbol=symbol, amount=amount)
                    time.sleep(1)
                    position['type'] = None 
                    bot.sendMessage(mc, "short 청산(4) : "+str(roe)+"%")
                    buy_phase=0 
                elif buy_condition == 9 and (slow_k_5m[-2] <= 20 or slow_k_5m[-1] <= 20) and (slow_k_1m[-2] <= 20 or slow_k_1m[-1] <= 20) and slow_k_1m[-2] < slow_d_1m[-2] and slow_k_1m[-1] >= slow_d_1m[-1] :
                    binance.create_market_buy_order(symbol=symbol, amount=amount)
                    time.sleep(1)
                    position['type'] = None 
                    bot.sendMessage(mc, "short 청산(5) : "+str(roe)+"%")
                    buy_phase=0 
                elif buy_condition == 9 and (slow_k_1m[-2] < 20 or slow_k_1m[-1] < 20) and slow_k_1m[-2] <= slow_d_1m[-2] and slow_k_1m[-1] > slow_d_1m[-1]  :
                    binance.create_market_buy_order(symbol=symbol, amount=amount)
                    time.sleep(1)
                    position['type'] = None 
