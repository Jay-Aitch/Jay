...
while True: 
    try:
        ...
        slow_k_3m = fast_k_3m.rolling(window=4).mean()
        slow_d_3m = slow_k_3m.rolling(window=4).mean()
        ...
        if ((position['type'] is None) or (position['type']=='long' and buy_phase <= 1 and roe < -5)) and bb == 1 :
                ...
                if slow_k_5m[-2] < 20 and buy_status == 0  :
                    if slow_k_5m[-1] >= slow_d_5m[-1] and slow_k_5m[-3] <= slow_k_5m[-2] <= slow_k_5m[-1]  :
                        if slow_k_3m[-3] < 80 and slow_k_3m[-3] <= slow_d_3m[-3] and slow_k_3m[-2] > slow_d_3m[-2] and slow_k_3m[-1] > slow_d_3m[-1]  :
                            ...
                            aa = df_5m['open'][-1]
                            buy_status = 1
                    # and slow_k_5m[-2] <= slow_k_5m[-1]  and macd_5m[-2] <= macd_5m[-1]and  macd_3m[-3] <= macd_3m[-2] <= macd_3m[-1] and macd_1m[-3] <= macd_1m[-2] <= macd_1m[-1]
                if slow_k_3m[-2] < 80 and buy_status == 0  :
                    if slow_k_3m[-1] >= slow_d_3m[-1] and slow_k_3m[-3] <= slow_k_3m[-2] <= slow_k_3m[-1]  :
                        if slow_k_5m[-3] < 20 and slow_k_5m[-3] <= slow_d_5m[-3] and slow_k_5m[-2] > slow_d_5m[-2] and slow_k_5m[-1] > slow_d_5m[-1]  :
                            position['type'] = 'long'
                            if buy_phase == 0 :
                                buy_phase = 1
            ...
            if slow_k_30m[-1] < 90 and macd_5m[-1] > macd_signal_5m[-1] and macd_5m[-2] < macd_5m[-1] and slow_d_5m[-2] < slow_d_5m[-1] :
                ...
                if slow_k_3m[-2] < 80 and buy_status == 0  :
                    if slow_k_3m[-1] >= slow_d_3m[-1] and slow_k_3m[-3] <= slow_k_3m[-2] <= slow_k_3m[-1]  :
                        if slow_k_5m[-3] < 80 and slow_k_5m[-3] <= slow_d_5m[-3] and slow_k_5m[-2] > slow_d_5m[-2] and slow_k_5m[-1] > slow_d_5m[-1]  :
                            position['type'] = 'long'
                            if buy_phase == 0 :
                                buy_phase = 1
                                buy_msg = "long 진입(1차):조건5"
                                amt = amount
                            elif buy_phase == 1 :
                                buy_phase = 2
                                buy_msg = "long 진입(2차):조건5"
                                amt = amount 
                            elif buy_phase == 2 :
                                buy_phase =3
                                buy_msg = "long 진입(3차):조건5"
                                amt = amount 
                            position['amount'] = amt
                            binance.create_market_buy_order(symbol=symbol, amount=amt)
                            buy_condition = 1 
        ...
        if ((position['type'] is None) or (position['type']=='short' and buy_phase <= 1 and roe < -5)) and bb == 1  :
            ...
            if slow_k_30m[-1] > 10 and slow_k_30m[-1] <= slow_d_30m[-1] and macd_30m[-2] >= macd_30m[-1]   :
                ...
                if slow_k_3m[-2] > 20 and buy_status == 0  :
                    if slow_k_3m[-1] <= slow_d_3m[-1] and slow_k_3m[-3] >= slow_k_3m[-2] >= slow_k_3m[-1]  :
                        if slow_k_5m[-3] > 80 and slow_k_5m[-3] >= slow_d_5m[-3] and slow_k_5m[-2] < slow_d_5m[-2] and slow_k_5m[-1] < slow_d_5m[-1] :
                            position['type'] = 'short'
                            if buy_phase == 0 :
                ...
                if slow_k_3m[-2] > 20 and buy_status == 0  :
                    if slow_k_3m[-1] <= slow_d_3m[-1] and slow_k_3m[-3] >= slow_k_3m[-2] >= slow_k_3m[-1]  :
                        if slow_k_5m[-3] > 20 and slow_k_5m[-3] >= slow_d_5m[-3] and slow_k_5m[-2] < slow_d_5m[-2] and slow_k_5m[-1] < slow_d_5m[-1] :
                            ...
                            if buy_phase == 0 :
                                buy_phase = 1
                                buy_msg = "short 진입(1차):조건5"
                                amt = amount
                            elif buy_phase == 1 :
                                buy_phase = 2
                                buy_msg = "short 진입(2차):조건5"
                                amt = amount 
                            elif buy_phase == 2 :
                                buy_phase = 3
                                buy_msg = "short 진입(3차):조건5"
                                amt = amount 
                            position['amount'] = amt
                            binance.create_market_sell_order(symbol=symbol, amount=amt)
                            buy_condition = 1 
