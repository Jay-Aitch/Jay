...
print("Upbit Auto-Trade START")
bot.sendMessage(mc, "업비트 자동매매 시작")

ticker = "EOS"
get_balance(ticker) 
msg_balance = "KRW잔고: "+str(int(krw))+"원\n"+ticker+"잔고: "+str(int(ticker_b))+"\n매수평단가: "+ str(int(ticker_avg_buy))+"원"
...
while True:
    try:
        ...
        if (slow_k[-2] >= 80 and slow_k[-2] >= slow_d[-2] and slow_k[-1] < slow_d[-1]) or (current_price > ticker_avg_buy*1.05) or (current_price < ticker_avg_buy*0.97)  :
        ...
        if (slow_k[-2] < slow_d[-2]) and (slow_k[-1] >= slow_d[-1]) and (macd_osc[-2] < macd_osc[-1]) and (slow_k_30m[-2] < slow_k_30m[-1]) and (krw > 5000):
